...
@interface PioerGiftDownloadSession () <NSURLSessionDataDelegate>

/** session 可以支持多个任务下载，创建一次就可以 */
@property (nonatomic,strong) NSURLSession *session;

/** 下载列队管理 专门负责接收到数据时分配给不同operation */
@property (nonatomic,strong) PioerGiftDownloadQueue *queue;

@end


@implementation PioerGiftDownloadSession
// 添加任务
- (void)downloadWithURL:(PioerdReceiveGiftData *)model begin:(void(^)(NSString *))begin progress:(void(^)(NSInteger,NSInteger))progress complete:(void(^)(NSDictionary *,NSError *))complet {
    // 交给列队管理
    [self.queue addDownloadWithSession:self.session URL:model begin:begin progress:progress complete:complet];
}
...
- (PioerGiftDownloadQueue *)queue {
    if (!_queue) {
        _queue = [[PioerGiftDownloadQueue alloc] init];
    }
    return _queue;
}
@end
