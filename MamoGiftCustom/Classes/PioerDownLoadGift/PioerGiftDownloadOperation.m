...
@interface PioerGiftDownloadOperation ()
...
@implementation PioerGiftDownloadOperation

- (instancetype)initWith:(PioerdReceiveGiftData *)model session:(NSURLSession *)session {
    
    if (self = [super init]) {
        _url = [NSURL URLWithString:model.animEffectUrl].absoluteString;
        _icon = model.icon;
        _name = model.name;
        ...
        // 初始化下载信息
        _currentSize = [self getFileSizeWithURL:[NSURL URLWithString:model.animEffectUrl].absoluteString];
        
        // 偏好设置里面存储总数据
        _totalSize = [PioerDownCacheManager totalSizeWith:[NSURL URLWithString:model.animEffectUrl].absoluteString];
        
        // 校验
        if (self.currentSize == self.totalSize && self.totalSize != 0 && [[PioerDownCacheManager updatedAtTimeWith:[NSURL URLWithString:model.animEffectUrl].absoluteString] isEqualToString:model.updatedAt]) {
            return nil;
        }
        // 删除之前存在的记录
        if (self.currentSize == self.totalSize && self.totalSize != 0 && ![[PioerDownCacheManager updatedAtTimeWith:[NSURL URLWithString:model.animEffectUrl].absoluteString] isEqualToString:model.updatedAt]) {
        ...
        _dataTask = [session sg_downloadDataTaskWithURLString:[NSURL URLWithString:model.animEffectUrl].absoluteString startSize:_currentSize];
    }
...
#pragma mark - PioerGiftDownloadOperationProtocol
// 接收到相应时
- (void)operateWithResponse:(NSURLResponse *)response {
    ...
    // 开始下载记录文件下载信息
    [PioerDownCacheManager saveFileInfoWithDict:[self downLoadInfoWithFinished:NO]];
...
- (void)completCusesseWithCode:(NSInteger)code {
    ...
    if (code == 1) {
        // 存储 文件下载信息
        [PioerDownCacheManager saveFileInfoWithDict:dict];
    }
